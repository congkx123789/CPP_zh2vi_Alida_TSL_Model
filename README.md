...
Bộ công cụ dịch thuật Trung - Việt **100% Nguyên bản C++ (Native C++)**, chạy độc lập dưới dạng file thực thi nhị phân (**Binary ELF**) không phụ thuộc vào Python hay bất kỳ thư viện môi trường cồng kềnh nào.

---

## 🏛️ KẾN TRÚC HỆ THỐNG DỊCH 3 TRẠM (3-STATION PIPELINE ARCHITECTURE)

Động cơ dịch **Alida TSL** được thiết kế theo kiến trúc 3 Trạm kết hợp giữa **Từ điển Tri thức C++ Zero-Copy** và **Mô hình Trí tuệ Nhân tạo Không Tự Hồi Quy (Non-Autoregressive INT8 Model)**:
...
## 🧠 CHI TIẾT KIẾN TRÚC MÔ HÌNH NEURAL (STUDENT NAT INT8 MODEL)
...
### 1. **Station 1: Dictionary & DAWG Trie DP Matcher (`dictionary.cpp`)**
- **MARISA DAWG Trie (`trie.marisa` - 6.0 MB)**: Nén **765,710 cụm từ** bằng đồ thị hữu hạn trạng thái bất biến (Directed Acyclic Word Graph), tra cứu độ phức tạp $O(L)$ theo độ dài từ.
- **Flat UTF-8 Byte Buffer (`meanings_flat.bin` - 12 MB)**: Lưu toàn bộ chuỗi nghĩa tiếng Việt nối tiếp trong mảng byte phẳng, truy cập zero-copy thông qua mảng offset 32-bit (`offsets.bin`).
...
### 2. **Station 2: Neural NAT INT8 Model (`onnx_engine.cpp`)**
- **Động cơ suy luận C++**: Dùng trực tiếp **ONNX Runtime C API (`onnxruntime_c_api.h`)** nạp file model **`student_nat_int8.onnx` (17 MB)**.
...
### 3. **Station 3: Logits Processor & Hán-Việt Fallback (`logits_processor.cpp`)**
...
- **Hán-Việt Fallback (`hanviet.bin` - 324 KB)**: Tự động tra cứu **24,474 từ Hán Việt** cho những chữ Hán ngoài từ điển Trạm 1, đảm bảo không bỏ sót bất kỳ ký tự nào.
...
## 📁 CẤU TRÚC THƯ MỤC DỰ ÁN (`TSL_CPP_Native/`)

```
TSL_CPP_Native/                  (Tổng dung lượng: ~39 MB)
├── build.sh                     # Script biên dịch C++ tự động
├── tsl_translator               # File nhị phân thực thi nguyên bản C++ (344 KB)
├── README.md                    # Tài liệu hướng dẫn & Kiến trúc chi tiết
│
├── src/                         # Toàn bộ mã nguồn C++ nguyên bản
│   ├── dictionary.hpp/cpp       # Trạm 1: Từ điển MARISA-Trie & DP matching C++
│   ├── tokenizer.hpp/cpp        # Bộ mã hóa/giải mã từ vựng Tokenizer C++
│   ├── logits_processor.hpp/cpp # Trạm 3: Bộ can thiệp Logits & Hán-Việt C++
│   ├── onnx_engine.hpp/cpp      # Trạm 2: Động cơ suy luận ONNX Runtime C API
│   ├── translator.hpp/cpp       # Lớp quản lý tổng thể TSLTranslator C++
│   └── main.cpp                 # Giao diện dòng lệnh CLI C++
│
├── data/                        # Tất cả dữ liệu đã nén Binary (Không có TXT)
│   ├── trie.marisa              # Từ điển MARISA-Trie DAWG (6.0 MB)
│   ├── meanings_flat.bin        # Bảng nghĩa UTF-8 Flat Store (12 MB)
│   ├── offsets.bin              # Mảng vị trí Offset 32-bit (2.0 MB)
│   ├── tiers.bin                # Mảng Metadata Tier (504 KB)
│   ├── zh_vocab.bin             # Từ vựng tiếng Trung Binary (224 KB)
│   ├── vi_vocab.bin             # Từ vựng tiếng Việt Binary (204 KB)
│   └── hanviet.bin              # Từ điển Hán Việt Binary (324 KB)
│
├── model/                       # Mô hình AI lượng hóa
│   └── student_nat_int8.onnx    # Model ONNX INT8 Quantized (17 MB)
│
└── lib/                         # Các thư viện liên kết động (.so) & Headers
    ├── marisa/                  # Thư viện tĩnh libmarisa.a & Headers
    └── onnxruntime/             # Physical Shared Library (.so) & Headers C API
```
...
```bash
cd /home/alida/Documents/My_model_translate/TSL_CPP_Native
./build.sh
```
